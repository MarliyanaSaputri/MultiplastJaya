...
<?php 
    if($_POST){
        $nama = $_POST['tanggal'];
        $email = $_POST['nik'];
        $alamat = $_POST['nama_karyawan'];
        $kota = $_POST['hasil_jadi'];
        $kodePos = $_POST['kd_barang'];
    $error = array();
        if(empty($nama)){
      $error['tanggal'] = 'Tanggal jangan kosong donk';
        }
    if(empty($email)){
      $error['nik'] = 'nik jangan kosong !!';
    }
    if(empty($alamat)){
      $error['nama_karyawan'] = 'Nama anda masi kosong';
    }
    if(empty($kota)){
      $error['hasil_jadi'] = 'hasil  kosong';
    }
    if(empty($kodePos)){
      $error['kd_barang'] = 'Kode  tolong di isi';
    }
    if(empty($error)){
            //prosess data
        }
...
<div class="col-md-12">
          ...
          <div class="box box-info">
            ...
            <!-- /.box-header -->
            <!-- form start -->
             <form class="form-horizontal" name='autoSumForm' method="post" id="frm_custumer" action="?page=simpanselector">
              <div class="box-body">
